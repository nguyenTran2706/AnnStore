...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!review) return res.status(404).json({ message: 'Review not found' });
      // Best-effort cleanup of uploaded photos (Blob in prod, disk in local dev)
      for (const url of review.images || []) {
        if (/^https?:\/\//.test(url)) {
          del(url).catch(() => {});
        } else {
          fs.unlink(path.join(__dirname, '../../uploads/reviews', path.basename(url))).catch(() => {});
        }
      }
...
/* The admin image gallery is served as a static manifest
   (client/public/images-manifest.json, generated by scripts/gen-image-manifest.js),
   so no serverless directory read is needed here. */
